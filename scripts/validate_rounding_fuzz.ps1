...
if ($libContent -match "mod test_rounding_fuzz;") {
    Write-Host "Test module included in lib.rs" -ForegroundColor Green
} else {
    Write-Host "Test module not included in lib.rs" -ForegroundColor Red
    exit 1
}
...
if ($cargoContent -match "proptest") {
    Write-Host "✓ proptest dependency found in Cargo.toml" -ForegroundColor Green
} else {
    Write-Host "✗ proptest dependency missing in Cargo.toml" -ForegroundColor Red
    exit 1
}
...
if (Test-Path $docFile) {
    Write-Host "✓ Documentation exists: $docFile" -ForegroundColor Green
} else {
    Write-Host "✗ Documentation missing: $docFile" -ForegroundColor Red
    exit 1
}
...
foreach ($test in $requiredTests) {
    if ($testContent -match $test) {
        Write-Host "✓ Found test function: $test" -ForegroundColor Green
    } else {
        Write-Host "✗ Missing test function: $test" -ForegroundColor Red
        exit 1
    }
}
...
foreach ($constant in $requiredConstants) {
    if ($testContent -match "const $constant") {
        Write-Host "✓ Found constant: $constant" -ForegroundColor Green
    } else {
        Write-Host "✗ Missing constant: $constant" -ForegroundColor Red
        exit 1
    }
}
...
foreach ($func in $requiredFunctions) {
    if ($testContent -match "fn $func") {
        Write-Host "✓ Found verification function: $func" -ForegroundColor Green
    } else {
        Write-Host "✗ Missing verification function: $func" -ForegroundColor Red
        exit 1
    }
}
...
if ($testContent -match "proptest!") {
    Write-Host "✓ Found proptest macro for fuzz testing" -ForegroundColor Green
} else {
    Write-Host "✗ Missing proptest macro" -ForegroundColor Red
    exit 1
}
...
Write-Host "✓ Total lines in test file: $lineCount" -ForegroundColor Green
Write-Host "✓ Number of test functions: $($requiredTests.Length)" -ForegroundColor Green
Write-Host "✓ Number of verification functions: $($requiredFunctions.Length)" -ForegroundColor Green
...
Write-Host "✓ Micro-stream rate: $microStreamRate stroops/day" -ForegroundColor Green
Write-Host "✓ Max error per stream: $maxErrorPerStream stroops" -ForegroundColor Green
Write-Host "✓ Total max error for $numStreams streams: $totalMaxError stroops ($([math]::Round($totalMaxError/10000000, 7)) XLM)" -ForegroundColor Green
...
try {
    ...
    if ($rustVersion) {
        ...
        Write-Host "⚠ Rust not found in PATH" -ForegroundColor Yellow
...
try {
    ...
    if ($cargoVersion) {
        ...
        Write-Host "⚠ Cargo not found in PATH" -ForegroundColor Yellow
    }
} catch {
    Write-Host "⚠ Cargo not found in PATH" -ForegroundColor Yellow
}
